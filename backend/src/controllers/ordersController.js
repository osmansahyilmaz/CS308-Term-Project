...
const placeOrder = async (req, res) => {
    ...
    try {
        // Step 1: Fetch the user's cart (including price & discount)
        const cartQuery = `
            SELECT c.product_id,
                   c.quantity,
                   p.in_stock,
        ...
        const orderItemsQuery = `
            INSERT INTO products_of_order
              (order_id, product_id, price_when_buy, tax_when_buy, discount_when_buy, product_order_count)
            VALUES ($1, $2, $3, $4, $5, $6)
        `;
        ...
        for (const item of cartResult.rows) {
            const priceWhenBuy    = parseInt(item.price, 10);
            const taxWhenBuy      = 0;                  // default tax, adjust as needed
            const discountWhenBuy = item.discount || 0; // snapshot of current discount
...
const cancelOrder = async (req, res) => {
    ...
    try {
        ...
        // Step 2: Allow cancellation only for statuses 0 (verifying) or 1 (processing)
        if (orderStatus > 1) {
        ...
        const restoreStockQuery = `
            UPDATE products
             ...
             WHERE po.product_id = products.product_id
               AND po.order_id = $1
        `;
